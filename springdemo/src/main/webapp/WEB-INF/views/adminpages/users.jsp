...
<script>
	function deleteUser(ids, empno) {
		if (confirm('確定要刪除會員編號為: ' + empno + ' 這筆紀錄?')) {
			let url = "<c:url value='/adminpages/user/UserDelete' />" + "/"
					+ ids;
	...
	function queryUserByKeyword(){
	    var keyword = document.getElementById('form1').value;
	    window.location.href = '/adminpages/user/queryUserByKeyword?keyword=' + keyword;
	}
						...
						<div class="col-md-12">
								...
								<tbody>
									...
									<c:forEach var='user' items='${userList}'>
										<tr>
											...
											<td>${user.password}</td>
											<td>${user.email}</td>
											<td><a class="edit" href=""> <i
													class="fa-sharp fa-light fa-pen" id="material-icons"></i>
											</a> <a class="delete"
												onclick="return deleteUser('${user.id}', '${user.username}');">
							...
							<div class="clearfix">
								<div class="hint-text">
									...
									筆，共有 <b id="total">${fn:length(userList)}</b> 筆
								</div>
								<ul class="pagination">
									<!-- 上一頁按鈕 -->
